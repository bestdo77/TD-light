...
const i18n = {
    zh: {
        ...
        train_title: '增量训练',
        train_upload: '上传光变曲线',
        train_upload_hint: '点击或拖拽上传',
        train_upload_support: '支持 CSV 或 ZIP',
        train_label: '类别标签',
        train_select_class: '-- 选择类别 --',
        train_do_train: '同时训练模型',
        train_do_train_hint: '勾选后会基于历史数据增量更新所有相关子模型',
        train_start: '开始',
        train_stop: '停止',
        train_progress: '训练进度',
        train_data_format: '数据格式说明',
        train_format_multi: '单文件多目标 CSV',
        train_format_multi_desc: '一个 CSV 包含多个天体的观测数据，必须包含 source_id 列用于分组。',
        train_format_zip: 'ZIP 压缩包',
        train_format_zip_desc: '每个 CSV 文件对应一个天体，文件名即为 source_id。打包成 ZIP 后上传。',
        train_col_ref: '字段说明',
        train_col_time: '观测时刻 (MJD/HJD/BJD)',
        train_col_mag: '星等 (mag)',
        train_col_err: '星等误差',
        train_col_sid: '目标标识 (多目标 CSV 必需)',
        train_format_tip: '提示：也支持标准格式 source_id,band,time,mag,mag_error,flux,flux_error。系统会自动识别列名（不区分大小写）。每个天体至少需要 5 个有效数据点。',
        train_summary: '训练数据概览',
        train_summary_empty: '暂无训练数据',
    ...
    en: {
        ...
        // Training Page
        nav_train: 'Training',
        train_title: 'Incremental Training',
        train_upload: 'Upload Light Curves',
        train_upload_hint: 'Click or drag to upload',
        train_upload_support: 'Supports CSV or ZIP',
        train_label: 'Class Label',
        train_select_class: '-- Select Class --',
        train_do_train: 'Train model after extraction',
        train_do_train_hint: 'Updates all relevant sub-models incrementally based on historical data',
        train_start: 'Start',
        train_stop: 'Stop',
        train_progress: 'Progress',
        train_data_format: 'Data Format',
        train_format_multi: 'Multi-object CSV',
        train_format_multi_desc: 'A single CSV containing observations for multiple objects. Must include a source_id column for grouping.',
        train_format_zip: 'ZIP Archive',
        train_format_zip_desc: 'Each CSV file corresponds to one object, with the filename as source_id. Pack into a ZIP and upload.',
        train_col_ref: 'Column Reference',
        train_col_time: 'Observation time (MJD/HJD/BJD)',
        train_col_mag: 'Magnitude (mag)',
        train_col_err: 'Magnitude error',
        train_col_sid: 'Object ID (required for multi-object CSV)',
        train_format_tip: 'Tip: The standard format source_id,band,time,mag,mag_error,flux,flux_error is also supported. Column names are auto-detected (case-insensitive). Each object needs at least 5 valid data points.',
        train_summary: 'Data Summary',
        train_summary_empty: 'No training data yet',
